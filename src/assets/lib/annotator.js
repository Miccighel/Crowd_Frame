	...
	preFilter: {
		...
		"PSEUDO": function( match ) {
			...
			// Strip excess characters from unquoted arguments
				...
				// excess is a negative document_index
			...
			return first === 1 && last === 0 ?
				...
				function( elem, context, xml ) {
					...
					if ( parent ) {
						...
						if ( forward && useCache ) {
							// Seek `elem` from a previously-cached document_index
						...
						// Use previously-cached element document_index if available
						...
						// xml :nth-child(...) or :nth-last-child(...) or :nth(-last)?-of-type(...)
							...
							while ( (node = ++nodeIndex && node && node[ dir ] ||
								...
								if ( ( ofType ? node.nodeName.toLowerCase() === name : node.nodeType === 1 ) && ++diff ) {
									// Cache the document_index of each encountered element
									if ( useCache ) {
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			...
			if ( bySet && i !== matchedCount ) {
				...
				if ( seed ) {
					...
					// Discard document_index placeholder values to get only actual matches
...
jQuery.fn.extend({
	...
	index: function( elem ) {
		...
		// No argument, return document_index in parent
		if ( !elem ) {
			return ( this[0] && this[0].parentNode ) ? this.first().prevAll().length : -1;
		}

		// document_index in selector
...
function propFilter( props, specialEasing ) {
	...
	for ( index in props ) {
		...
		if ( hooks && "expand" in hooks ) {
			...
			// also - reusing 'document_index' from above because we have the correct "name"
			for ( index in value ) {
				if ( !( index in props ) ) {
	...
	if ( jQuery.isArray( obj ) ) {
		...
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				...
				// Item is non-scalar (array or object), encode its numeric document_index.
...
// maxZIndex returns the maximum z-document_index of all elements in the provided set.
function maxZIndex(elements) {
    ...
    for (var i = 0, len = elements.length; i < len; i++) {
        ...
        if ($el.css('position') !== 'static') {
            ...
            // values.
            var zIndex = parseFloat($el.css('z-document_index'));
            if (zIndex > max) {
                max = zIndex;
            }
        }
...
// are displayed with the highest z-document_index.
function injectDynamicStyle() {
    ...
    // use the maximum z-document_index in the page
    ...
    var rules = [
        ...
        "  z-document_index: " + (max + 20) + ";",
        "}",
        ".annotator-filter {",
        "  z-document_index: " + (max + 10) + ";",
        "}"
    ].join("\n");
