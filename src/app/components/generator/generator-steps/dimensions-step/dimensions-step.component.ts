...
export class DimensionsStepComponent implements OnInit {
    ...
    public async ngOnInit() {
        let serializedDimensions = Object.keys(localStorage).filter((key) =>
        ...
        if (serializedDimensions.length > 0) {
            serializedDimensions.forEach((key) => {
                ...
                this.dataStored.push(
                    new Dimension(parseInt(index), JSON.parse(item))
                );
            });
            this.dataStored.sort((a, b) => (a.index > b.index ? 1 : -1));
        } else {
            this.initializeControls();
            let rawDimensions = await this.S3Service.downloadDimensions(
        ...
        if (dimension) {
            if (dimension.justification) {
                justification = this._formBuilder.group({
                    ...
                    min_words: [
                        dimension.justification.min_words,
                        [Validators.required],
                    ],
                });
            }
            ...
            if (dimension.scale) {
                let scaleConfig = {
                    ...
                    setMultipleSelection:
                        !!dimension.scale["multiple_selection"],
                    multiple_selection: dimension.scale["multiple_selection"]
                        ? dimension.scale["multiple_selection"]
                        : false,
                    setInstructions: !!dimension.scale["instructions"],
                    min: dimension.scale["min"]
                        ? [dimension.scale["min"], [Validators.required]]
                        : "",
                    max: dimension.scale["max"]
                        ? [dimension.scale["max"], [Validators.required]]
                        : "",
            ...
            if (dimension.scale)
                if (dimension.scale.type == "categorical") {
                    if (dimension.scale["mapping"])
                        for (let mapping of dimension.scale["mapping"])
    ...
    removeDimensionMapping(
        dimensionIndex: number,
        dimensionMappingIndex: number
    ) {
        ...
        dimensionsJSON.forEach((dimension, dimensionIndex) => {
            ...
            if (dimension.setScale == false) {
                ...
                if (dimension.scale.setInstructions == false) {
                    ...
                    if (dimension.scale.instructions.caption == "")
                        dimension.scale.instructions.caption = false;
                    if (dimension.scale.instructions.label == "")
                        dimension.scale.instructions.label = false;
                    if (dimension.scale.instructions.text == "")
                        dimension.scale.instructions.text = false;
                }
            ...
            if (dimension.style) {
                if (dimension.setStyle) {
                    ...
                    if (
                        dimension.scale.type == "interval" ||
                        dimension.scale.type == "magnitude_estimation"
                    ) {
