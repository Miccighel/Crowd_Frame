...
export class ActionLogger {
    ...
    /**
     * Resolve the effective logging endpoint:
     * 1. task.json server_endpoint if set
     * 2. environment.log_endpoint (from deployment script)
     */
    private resolveEndpoint(): string | null {
        ...
        const env: any = this.configService.environment;
        if (env && env.log_endpoint && env.log_endpoint.trim() !== "") {
            return env.log_endpoint.trim();
        }
    ...
    log(type: string, details: object) {
        const payload = this.buildPayload(type, details);

        // If logger is disabled, optionally still log to console and exit
        if (!this._isActive) {
        ...
        if (!endpoint) {
            console.warn("[ActionLogger] No logging endpoint configured. Event:", payload);
            return;
        }
     ...
     */
    windowResize(obj: any) {
        let details: any = this.getCurrentSize();
    ...
    onResult(results: any) {
        let urlArray: string[] = [];
        results["decodedResponses"].forEach((result: any) => {
