    ...
    for worker_id in tqdm.tqdm(worker_identifiers):
        ...
        if not os.path.exists(worker_snapshot_path):
            ...
            for data_source, worker_session in worker_data.items():
                    ...
                    if data:
                        ...
                        if data['info']['element'] == 'data':
                            ...
                            if 'settings' in snapshot['task']:
                                ...
                                snapshot['task']['settings'] = data['info']
                ...
                if len(snapshot['documents']) > 0 or len(snapshot['questionnaires']) > 0:
...
def find_snapshot_for_record(acl_record):
    if acl_record['source_path'] is not np.nan:
        snapshots = read_json(acl_record['source_path'])
        ...
        if type(snapshots) == dict:
            ...
            for snapshot in snapshots:
                if 'task' in snapshot:
                    task = snapshot['task']
                    if 'task_id' in task:
                        if snapshot['task']['worker_id'] == acl_record['worker_id'] and snapshot['task']['task_id'] == acl_record['task_name']:
                                snapshots_found.append(snapshot)
                    elif 'task_name' in task:
                        if snapshot['task']['worker_id'] == acl_record['worker_id'] and snapshot['task']['task_name'] == acl_record['task_name']:
                                snapshots_found.append(snapshot)
        else:
            assert False
        ...
        for pool in toloka_client.find_pools(project_id=project_data.id, sort=['last_started']).items:
            ...
            for task_suite in tqdm.tqdm(task_suites, desc=f"Processing task suites for pool {pool.id}:"):
                ...
                for assignment in toloka_client.find_assignments(task_suite_id=task_suite.id, sort=['created']).items:
                    ...
                    if 'assignment_token_input_final' in acl_rows:
                        acl_rows = acl_rows.loc[acl_rows['assignment_token_input_final']==row['assignment_token_input_final']]
                    if len(acl_rows) > 1:
            ...
            for snapshot in snapshots:
                ...
                if 'logs' in snapshot:
                    ...
                    if len(logs) > 0:
                        ...
                        if len(snapshot['documents_answers']) or len(snapshot['questionnaires_answers']) > 0:
                            task_started = True
...
if not os.path.exists(df_quest_path):
    ...
    for index, acl_record in tqdm.tqdm(df_acl.iterrows(), total=df_acl.shape[0]):
        ...
        for worker_snapshot in snapshots:
            ...
            documents_answers = worker_snapshot['questionnaires_answers']
