...
console.rule("Final checks")
with console.status("Checking user policies...", spinner="aesthetic") as status:
    denied = []
...
if len(keys) < 2:
    ...
    serialize_json(folder_aws_path, f"user_{user['User']['UserName']}_access_key_{key['AccessKey']['AccessKeyId']}.json",
                   key)
    ...
    if not os.path.exists(
        ...
        serialize_json(folder_aws_path,
                       f"user_{user['User']['UserName']}_access_key_{key['AccessKey']['AccessKeyId']}.json", key)
        console.print(f"[green]Access key created[/green], HTTP STATUS CODE: {key['ResponseMetadata']['HTTPStatusCode']}.")

key_selected = random.choice(keys)
key_data = read_json(
    f"{folder_aws_path}user_{user['User']['UserName']}_access_key_{key_selected['AccessKey']['AccessKeyId']}.json")
...
if server_config:
    console.rule("AWS logging server setup")
    with console.status("Policies setup...", spinner="aesthetic") as status:
        time.sleep(2)
        path = '/crowdFrame/'
...
console.rule(
    f"Task [cyan underline]{task_name}[/cyan underline]/[yellow underline]{batch_name}[/yellow underline] build")
...
console.rule(
    f"Task [cyan underline]{task_name}[/cyan underline]/[yellow underline]{batch_name}[/yellow underline] deploy")
